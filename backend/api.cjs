...
const safeUser=u=>({id:u.id,name:u.name,empresa:u.empresa,role:u.role,enabled:u.enabled});
...
async function bootstrap(store,{name,pass,empresa='RAS'}){requireThat(companies.includes(empresa),'Empresa inválida.');const c=await credentials(pass);await store.transaction(db=>{requireThat(!db.users.length,'El administrador inicial ya existe.');db.companies[empresa]={revision:0,enabled:true,state:{trains:[],history:[],locomotives:[],crew:[]},audit:[]};db.users.push({id:crypto.randomUUID(),name,empresa,role:'Administrador',enabled:true,...c});});}
  ...
  try{
   ...
   if(action==='login'){
    ...
    const result=await store.transaction(async db=>{
     ...
     const u=db.users.find(u=>u.empresa===empresa&&u.name.toLowerCase()===name.toLowerCase());
     if(!u?.enabled||!db.companies[empresa]?.enabled||!await verify(u,body.pass)){db.attempts[k]={count:(attempt?.count||0)+1,until:attempt?.until||now+900000};return {error:'Credenciales incorrectas o cuenta deshabilitada.',status:401};}
   ...
   const result=await store.transaction(async db=>{
    ...
    if(action==='admin'&&req.method==='GET'){requireThat(u.role==='Administrador','Solo Administración.',403);return {users:db.users.map(safeUser),companies:Object.entries(db.companies).map(([id,c])=>({id,enabled:c.enabled}))};}
    if(action==='admin'&&req.method==='POST'){
     ...
     if(body.operation==='company'){requireThat(companies.includes(body.empresa),'Empresa inválida.');const target=db.companies[body.empresa]||={revision:0,enabled:true,state:{trains:[],history:[],crew:[],locomotives:[]},audit:[]};requireThat(body.empresa!==u.empresa||body.enabled!==false,'No podés deshabilitar tu propia empresa.');target.enabled=body.enabled!==false;}
      ...
      target.role=body.role;target.enabled=body.enabled!==false;
    ...
    if(action==='backup'&&req.method==='GET'){requireThat(u.role==='Administrador','Solo Administración.',403);return {format:'ferrosync-backup',version:1,...backup(db,u.empresa,'Exportación manual')};}
    ...
    if(action==='restore'){
     ...
     db.users=db.users.filter(x=>x.empresa!==u.empresa).concat(structuredClone(file.users.filter(x=>x.id!==u.id&&x.name.toLowerCase()!==u.name.toLowerCase())),u);
